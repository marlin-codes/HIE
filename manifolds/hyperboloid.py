...
class Hyperboloid(Manifold):
    ...
    #     mx = self.proj_tan0(mx, c)
    #     mx = self.expmap0(mx, c)
    ...
    def mid_point(self, x, c, w=None):
        if w is not None:
            ave = w.matmul(x)
        else:
            ave = x.mean(dim=-2)
        denom = (-self.minkowski_dot(ave, ave, keepdim=True)).clamp_min(1e-8).sqrt()
        return torch.sqrt(c) * ave / denom


    def init_weights(self, w, c, irange=1e-5):
        w.data.uniform_(-irange, irange)
        return torch.cat([torch.zeros(w.size(0), 1).to(w.device), w], dim=-1)