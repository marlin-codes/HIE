...
from layers.layers import GraphConvolution, Linear, SAGEConvolution
...
class SAGEDecoder(Decoder):
    ...
    def __init__(self, c, args):
        ...
        act = lambda x: x
        self.cls = SAGEConvolution(args.dim, args.n_classes, args.dropout, act, args.bias)
        self.decode_adj = True
...
class LGCNDecoder(Decoder):
    """
    MLP Decoder for Hyperbolic/Euclidean node classification models.
    """

    def __init__(self, c, args):
        super(LGCNDecoder, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
    ...
    def decode(self, x, adj):
        h = self.manifold.proj_tan0(self.manifold.logmap0(x, c=self.c), c=self.c)
        return super(LGCNDecoder, self).decode(h, adj)
